...
def send_telegram_alert(message: str, is_urgent: bool = False) -> bool:
    ...
    try:
        ...
        prefix = "🚨 URGENT: " if is_urgent else "ℹ️ "
...
def test_telegram_connectivity() -> bool:
    ...
    """
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        logger.warning("⚠️  Telegram not configured - alerts will be disabled")
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"❌ Telegram bot token invalid (status: {response.status_code})")
            return False
        
        bot_info = response.json()
        if not bot_info.get('ok'):
            logger.error(f"❌ Telegram API error: {bot_info.get('description', 'Unknown error')}")
            return False
        ...
        # Send test message
        test_message = "Guardian startup self-test - Telegram connectivity verified ✓"
        success = send_telegram_alert(test_message, is_urgent=False)
        
        if success:
            logger.info("✓ Telegram self-test PASSED - alerts enabled")
            return True
        else:
            logger.error("❌ Telegram self-test FAILED - check chat ID and permissions")
            return False
            
    except requests.exceptions.Timeout:
        logger.error("❌ Telegram connection timeout - check network connectivity")
        return False
    except requests.exceptions.ConnectionError:
        logger.error("❌ Telegram connection error - check network/firewall")
        return False
    except Exception as e:
        logger.error(f"❌ Telegram self-test failed: {e}")
        return False
...
class QuestDBGuardian:
    ...
    def validate_setup(self):
        ...
        if telegram_ok:
            logger.info("✓ Telegram alerts enabled and tested")
        else:
            logger.warning("⚠️  Telegram alerts disabled - Guardian will run without notifications")
        logger.info("=" * 60)
    ...
    def detect_wal_corruption(self, table_name: str) -> Tuple[bool, str]:
        """
        Detect table health issues using functional checks
        ...
        try:
            ...
            # Check 2: txn_seq directory exists (but give grace period for new tables)
            ...
            if not os.path.exists(txn_seq_path):
                ...
                scan_duration.labels(table=table_name).observe(time.time() - scan_start)
                return True, "txn_seq directory missing after grace period"
            
            # Check 3: Critical files exist
            critical_files = ['_txnlog', '_meta']
            for critical_file in critical_files:
                file_path = os.path.join(txn_seq_path, critical_file)
                if not os.path.exists(file_path):
                    scan_duration.labels(table=table_name).observe(time.time() - scan_start)
                    return True, f"Critical file missing: {critical_file}"
            
            # Check 4: Table responds to queries (functional check)
            try:
                ...
                if response.status_code != 200:
                    scan_duration.labels(table=table_name).observe(time.time() - scan_start)
                    return True, f"Table query failed with status {response.status_code}"
                ...
                scan_duration.labels(table=table_name).observe(time.time() - scan_start)
                return True, "Table query timeout"
            except requests.exceptions.ConnectionError:
                scan_duration.labels(table=table_name).observe(time.time() - scan_start)
                return True, "QuestDB connection error"
            
        except Exception as e:
            scan_duration.labels(table=table_name).observe(time.time() - scan_start)
            logger.error(f"Error detecting issues in {table_name}: {e}")
    ...
    def stop_services(self):
        ...
        for service in self.config['services_to_stop']:
            ...
            if status == 'inactive':
                logger.info(f"⊘ {service} already stopped, skipping")
                continue
            elif status == 'unknown':
                logger.warning(f"⚠ Cannot determine status of {service}, attempting stop anyway")
    ...
    def verify_table_health(self, table_name: str, max_wait: int = 30) -> Tuple[bool, str]:
        ...
        while (time.time() - start_time) < max_wait:
            ...
            try:
                ...
                if response.status_code == 200:
                    logger.info(f"✓ {table_name} is healthy and responsive")
    ...
    def check_circuit_breaker(self, table_name: str) -> Tuple[bool, str]:
        ...
        if recent_attempts >= max_attempts:
            ...
            if recent_successes == 0:
                ...
                logger.error(
                    f"🚨 Circuit breaker OPEN for {table_name}: "
                ...
                logger.warning(
                    f"⚠️ {table_name} has {recent_attempts}/{max_attempts} attempts "
    ...
    def record_recovery_attempt(self, table_name: str, success: bool):
        ...
        logger.info(f"📊 Recorded recovery attempt for {table_name}: {status}")
    ...
    def execute_emergency_recovery(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info("✓ Emergency script completed successfully")
                logger.info(f"Output: {result.stdout}")
                return True
            else:
                logger.error(f"✗ Emergency script failed: {result.stderr}")
                return False
    ...
    def recover_from_corruption(self, table_name: str, corruption_type: str) -> bool:
        ...
        if not can_recover:
            logger.error(f"🚫 Circuit breaker preventing recovery: {breaker_reason}")
            send_telegram_alert(
        ...
        logger.info(f"✓ Circuit breaker check passed: {breaker_reason}")
        
        self.recovery_in_progress = True
        recovery_start = time.time()
        
        try:
            logger.critical(f"⚠️  STARTING RECOVERY for {table_name}: {corruption_type}")
            ...
            if recovery_success:
                ...
                if not is_healthy:
                    logger.error(f"✗ Health verification failed: {health_reason}")
                    recovery_success = False
                else:
                    logger.info(f"✓ Health verification passed: {health_reason}")
            ...
            if recovery_success:
                ...
                logger.error(f"✗ Recovery failed after {recovery_duration:.1f}s")
    ...
    def monitor_loop(self):
        ...
        logger.info("=" * 80)
        logger.info("🛡️  QuestDB Guardian Starting - Phase 2 Enhanced")
        logger.info("=" * 80)
        ...
        logger.info(f"📋 Monitoring tables: {', '.join(self.config['tables'])}")
        logger.info(f"🔧 Phase 2 Features: Health Verification ✓ | Circuit Breaker ✓ | Enhanced Logging ✓")
        logger.info("=" * 80)
                ...
                for table_name, (corrupted, reason) in results.items():
                    if corrupted:
                        ...
                        if consecutive_corruption_count[table_name] == 1:
                            logger.critical(f"🚨 CORRUPTION DETECTED: {table_name} - {reason}")
                            send_telegram_alert(
                                f"⚠️ Potential corruption detected in table '{table_name}': {reason}\n\n"
                                f"Guardian is in ALERT-ONLY mode. Manual investigation required.\n"
                                f"Check: journalctl -u questdb-guardian -n 100",
                                is_urgent=True
                            )
                            logger.warning(f"⚠️  ALERT-ONLY MODE: No automatic recovery attempted")
                        
                        elif consecutive_corruption_count[table_name] >= 3:
                            logger.critical(
                                f"🚨 PERSISTENT ISSUE: {table_name} flagged "
                                f"{consecutive_corruption_count[table_name]} times consecutively"
                            )
                            send_telegram_alert(
                                f"🚨 PERSISTENT: Table '{table_name}' flagged {consecutive_corruption_count[table_name]} times.\n"
                                f"Reason: {reason}\n"
