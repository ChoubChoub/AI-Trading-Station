...
%if %{with akmod}
%package akmod
Summary:          Onload kernel modules as Akmod source
...
%if %{with devel}
%package devel
Summary:          Onload development header files
...
%if %{with dkms}
%package dkms
Summary:          Onload kernel modules as DKMS source
Group:            System Environment/Kernel
Requires:         dkms >= 2.6
Provides:         openonload-kmod = %{version}-%{release}
Provides:         sfc-kmod-symvers = %{version}-%{release}
